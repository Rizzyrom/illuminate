...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // For now, use static data until TinaCMS is fully configured
        const staticData = {
          settings: {
            data: {
              settings: {
  ...
  if (pageData.settings?.data?.settings?.site?.maintenanceMode) {
    return <MaintenancePage data={pageData.settings.data.settings} />;
  }

  return (
    <div className="min-h-screen">
      <Navigation data={pageData.navigation.data.navigation} />
      <Hero data={pageData.hero.data.hero} />
      <Services data={pageData.services.data.services} />
      <Process data={pageData.process.data.process} />
      <About data={pageData.about.data.about} />
      <Contact data={pageData.contact.data.contact} />
      <Footer />
    </div>
  );
}
